    ...
    const insertData = async (detailsData, city) => {
      ...
      for (const [index, restaurant] of detailsData.entries()) {
        ...
        const detailsValues = [
          ...
          restaurant.photo ? JSON.stringify(restaurant.photo) : null,
          restaurant.awards ? JSON.stringify(restaurant.awards) : null,
          ...
          restaurant.hours ? JSON.stringify(restaurant.hours) : null,
          ...
          restaurant.cuisine ? JSON.stringify(restaurant.cuisine) : null,
          restaurant.dietary_restrictions ? JSON.stringify(restaurant.dietary_restrictions) : null,
          restaurant.booking ? JSON.stringify(restaurant.booking) : null,
          restaurant.reserve_info ? JSON.stringify(restaurant.reserve_info) : null,
          restaurant.menu_web_url || null,
          restaurant.establishment_category_ranking || null,
          restaurant.meal_types ? JSON.stringify(restaurant.meal_types) : null,
          restaurant.establishment_types ? JSON.stringify(restaurant.establishment_types) : null,
          restaurant.dishes ? JSON.stringify(restaurant.dishes) : null,
          restaurant.sub_cuisine ? JSON.stringify(restaurant.sub_cuisine) : null,
          restaurant.storyboard ? JSON.stringify(restaurant.storyboard) : null,
          restaurant.owners_top_reasons ? JSON.stringify(restaurant.owners_top_reasons) : null,
          restaurant.photo_count || null,
          restaurant.has_review_draft || false,
          restaurant.has_panoramic_photos || false,
          restaurant.rating_histogram ? JSON.stringify(restaurant.rating_histogram) : null,
        ];

        await client.query(`
          INSERT INTO details (
            location_id, name, latitude, longitude, num_reviews, timezone, location_string, photo,
            awards, doubleclick_zone, preferred_map_engine, raw_ranking, ranking_geo, ranking_geo_id,
            ranking_position, ranking_denominator, ranking_category, ranking, distance, distance_string,
            bearing, rating, is_closed, open_now_text, is_long_closed, price_level, price, description,
            web_url, write_review, phone, website, email, address, hours, local_name, local_address,
            local_lang_code, is_candidate_for_contact_info_suppression, cuisine, dietary_restrictions,
            booking, reserve_info, menu_web_url, establishment_category_ranking, meal_types, establishment_types,
            dishes, sub_cuisine, storyboard, owners_top_reasons, photo_count, has_review_draft, has_panoramic_photos,
            rating_histogram
          ) VALUES (
